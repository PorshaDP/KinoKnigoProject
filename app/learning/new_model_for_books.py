...
# Пути к файлам
data_path = os.path.join(os.path.dirname(__file__), 'filtered_books.csv')
model_path = os.path.join(os.path.dirname(__file__), 'recommender_books.pkl')
vectorizer_path = os.path.join(os.path.dirname(__file__), 'vectorizer_books.pkl')
...
def recommend_books(book_title, num_recommendations=10):
    ...
    try:
        ...
        recommendations = recommended_books[['title', 'authors', 'categories', 'published_year', 'average_rating', 'description']].to_dict(orient='records')
